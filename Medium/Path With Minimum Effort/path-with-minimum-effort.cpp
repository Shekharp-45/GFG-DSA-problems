  ...
  public:
    int MinimumEffort(int n, int m, vector<vector<int>> &heights) {
        ...
        while(!pq.empty()){
            ...
            int nrow, ncol;
            //it is for above(vr) jane from that point
            nrow=row-1; ncol=col;
            ...
            //it is for down(khali) jane from that point
            nrow=row+1; ncol=col;
            if(nrow>=0 && ncol>=0 && nrow<n && ncol<m){
                int val=max(step, abs(heights[nrow][ncol]-heights[row][col]));
                if(val<dist[nrow][ncol]){
            ...
            //it is for left jane from that point
            nrow=row; ncol=col-1;
            if(nrow>=0 && ncol>=0 && nrow<n && ncol<m){
                int val=max(step, abs(heights[nrow][ncol]-heights[row][col]));
                if(val<dist[nrow][ncol]){
            ...
            //it is for right jane from that point
            nrow=row; ncol=col+1;
            if(nrow>=0 && ncol>=0 && nrow<n && ncol<m){
                int val=max(step, abs(heights[nrow][ncol]-heights[row][col]));
                if(val<dist[nrow][ncol]){
