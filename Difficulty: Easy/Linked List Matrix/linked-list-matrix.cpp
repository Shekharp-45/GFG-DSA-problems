...
class Solution {
  public:
    Node* constructLinkedMatrix(vector<vector<int>>& mat) {
         ...
         int n=mat.size();
        vector<vector<Node*>>EleToNode(n,vector<Node*>(n,NULL));
       //making new vector of Node
        for(int i=0;i<n;i++){
            for(int j=0;j<n;j++){
                EleToNode[i][j]=new Node(mat[i][j]);
            }
        }
        //traverse & linking
        for(int i=0;i<n;i++){
            for(int j=0;j<n;j++){
                if (j + 1 < n) {
                EleToNode[i][j]->right = EleToNode[i][j + 1]; // Linking right
            }
            if (i + 1 < n) {
                EleToNode[i][j]->down = EleToNode[i + 1][j];  // Linking down
            }
            }
        }
        return EleToNode[0][0];
    }
};
